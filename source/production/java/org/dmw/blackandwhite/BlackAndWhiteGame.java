...
public class BlackAndWhiteGame
{
    ...
    private static final Map<Long, BlackAndWhiteGame> activeGames = new Hashtable<>();
    ...
    private BlackAndWhiteGame(long id, String player1, String player2)
    {
        this.id = id;
    ...
    public synchronized void move(Player player, int row, int column)
    {
        ...
        if( r1 == r2 ){
            int n1 = GetMove(BlackAndWhiteGame.Player.PLAYER1,r1);
            int n2 = GetMove(BlackAndWhiteGame.Player.PLAYER2,r2);
            if(n1>n2){
        ...
        if(this.getWinner() != null || this.isDraw())
            this.over = true;
        if(this.isOver())
            BlackAndWhiteGame.activeGames.remove(this.id);
    }
    ...
    {
        return (Map<Long, String>) BlackAndWhiteGame.pendingGames.clone();
    }

    public static long queueGame(String user1)
    {
        long id = BlackAndWhiteGame.gameIdSequence++;
        BlackAndWhiteGame.pendingGames.put(id, user1);
        return id;
    }

    public static void removeQueuedGame(long queuedId)
    {
        BlackAndWhiteGame.pendingGames.remove(queuedId);
    }

    public static BlackAndWhiteGame startGame(long queuedId, String user2)
    {
        String user1 = BlackAndWhiteGame.pendingGames.remove(queuedId);
        BlackAndWhiteGame game = new BlackAndWhiteGame(queuedId, user1, user2);
        BlackAndWhiteGame.activeGames.put(queuedId, game);
        return game;
    }

    public static BlackAndWhiteGame getActiveGame(long gameId)
    {
        return BlackAndWhiteGame.activeGames.get(gameId);
    }
